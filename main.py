...
def hparam_tuning(param: Dict, ht_param: Dict[str, Dict]) -> None:
    ...
    def hparam_optim(param: Dict, ht_param: Dict[str, Dict], trial: optuna.Trial) -> float:
        ...
        def get_suggestion(param_name: str, param_config: Dict):
            suggest_method = SUGGEST_METHOD_MAP[param_config['type']]
            sm_kwargs = {k: v for k, v in param_config.items() if k != 'type'}
            return suggest_method(param_name, **sm_kwargs)

        def handle_hidden_layer(attr):
            num_layers = get_suggestion('num_layers', attr['num_layers'])
            hidden_layer = [
                get_suggestion(f'neuron_{i}', attr['neuron']) for i in range(num_layers)
            ]
            return hidden_layer
        ...
        for hparam, attr in ht_param.items():
            if hparam == 'optuna':
                continue
            if hparam == 'hidden_layer':
                param['hidden_layer'] = handle_hidden_layer(attr)
            else:
                param[hparam] = get_suggestion(hparam, attr)
